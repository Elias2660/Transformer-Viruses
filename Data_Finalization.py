...
virus_data = pd.read_excel(
    "/content/drive/MyDrive/Transformer-Viruses/data/Woolhouse_and_Brierley_RNA_virus_database.xlsx"
)
...
while True:
    compValues.append(
        df.SequenceMatcher(None, virus_data.iloc[i, 0], genome_data.iloc[j, 2]).ratio()
    )
    i += 1
    if i == 213:
        i = 0
        ...
        norm_compValues = [
            (float(i) - min(compValues)) / (max(compValues) - min(compValues))
            for i in compValues
        ]
...
for j in range(1620):
    while True:
        compValues.append(
            df.SequenceMatcher(
                None, virus_data.iloc[i, 0], genome_data.iloc[j, 2]
            ).ratio()
        )
        i += 1
        if i == 213:
            ...
            for k in range(213):
                if norm_compValues[k] > 0.75:
                    genome_data.loc[j, "Estimated Transmission Level"] = (
                        virus_data.iloc[k, 20]
                    )
                    break
            break
    compValues = []
